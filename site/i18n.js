...
const COPY = {
  ru: {
    ...
    chat: {
      ...
      statusClosedSupport: 'Чат закрыт. Напишите в hello@reyz.app',
  ...
  uz: {
    ...
    chat: {
      ...
      statusClosedSupport: 'Chat yopilgan. hello@reyz.app manziliga yozing',
...
function applyAttributeTranslations(language) {
  for (const binding of ATTRIBUTE_BINDINGS) {
    ...
    document.querySelectorAll(binding.selector).forEach((node) => {
      ...
      if (binding.attr === 'data-src' && node.getAttribute('src')) {
